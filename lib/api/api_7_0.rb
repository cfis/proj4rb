module Proj
  module Api
    ...
    callback :open_cbk, [:PJ_CONTEXT, :string, PROJ_OPEN_ACCESS, :pointer], :PROJ_FILE_HANDLE

    # Read sizeBytes into buffer from current position and return number of bytes read
    callback :read_cbk, [:PJ_CONTEXT, :PROJ_FILE_HANDLE, :pointer, :size_t, :pointer], :size_t

    # Write sizeBytes into buffer from current position and return number of bytes written
    callback :write_cbk, [:PJ_CONTEXT, :PROJ_FILE_HANDLE, :pointer, :size_t, :pointer], :size_t

    # Seek to offset using whence=SEEK_SET/SEEK_CUR/SEEK_END. Return TRUE in case of success
    callback :seek_cbk, [:PJ_CONTEXT, :PROJ_FILE_HANDLE, :long_long, :int, :pointer], :int

    # Return current file position
    callback :tell_cbk, [:PJ_CONTEXT, :PROJ_FILE_HANDLE, :pointer], :long_long

    # Close file
    callback :close_cbk, [:PJ_CONTEXT, :PROJ_FILE_HANDLE, :pointer], :void

    # Return TRUE if a file exists
    callback :exists_cbk, [:PJ_CONTEXT, :string, :pointer], :int

    # Return TRUE if directory exists or could be created
    callback :mkdir_cbk, [:PJ_CONTEXT, :string, :pointer], :int

    # Return TRUE if file could be removed
    callback :unlink_cbk, [:PJ_CONTEXT, :string, :pointer], :int

    # Return TRUE if file could be renamed
    callback :rename_cbk, [:PJ_CONTEXT, :string, :string, :pointer], :int
    ...
    attach_function :proj_context_set_fileapi, [:PJ_CONTEXT, PROJ_FILE_API, :pointer], :int
